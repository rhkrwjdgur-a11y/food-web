...
# ==========================================
# 4-2. 실시간 AI 비전 분석 로직 [최종 범용 룰셋 & 오타 색출 통합]
# ==========================================
def analyze_design_with_ai(main_images, ref_files, master_fact_files, legal_text, db_context_text):
    ...
    prompt = f"""
    당신은 엄격하면서도 유연한 품질관리(QC) 전문가입니다. 제공된 시안 조각과 팩시안을 대조 검토하십시오.
    ...
    🚨 0. [환각 방지 및 연출 컷 주의문구]: 
       - 글자(OCR)를 당신의 뇌피셜로 보정하여 상상해 읽지 마십시오. 이미지에 적힌 그대로 읽으십시오.
       - 조리/연출 이미지 주변에 '연출된 이미지' 등의 주의문구가 누락되어 있다면 지적하십시오.

    🔥 1. [디자이너 오타(Typo) 색출 강제]: (가장 중요)
       - 영양정보표 내의 수치와 1일 영양성분 기준치 비율(%) 간의 논리적 모순을 무조건 찾아내십시오. 
       - 특히 함량이 **'0mg' 또는 '0g'인데 비율이 '0%'가 아닌 다른 수치(예: 33%)로 오기재**되어 있거나, 트랜스지방의 단위가 'g'이 아닌 'mg'으로 적혀 있다면 이는 디자이너의 복붙 오타이므로 "치명적 위반(오기재)"으로 적발하십시오.

    🔥 2. [의무표시와 기만표시의 명확한 구분]:
       - 영양정보표(표 형태) 안에 의무적으로 기재되는 '콜레스테롤 0mg'은 합법적 의무사항이므로 절대 기만이나 과대광고로 지적하지 마십시오. (정상 처리)
       - 단, 표 밖의 마케팅 카피에서 식물성 제품임에도 불구하고 '콜레스테롤 걱정 NO' 등 원래 없는 성분을 마치 특별히 뺀 것처럼 기만적으로 강조했다면, 면책 문구가 있어도 "치명적 위반"으로 적발하십시오.

    🔥 3. [부당 비교 금지 면책 무시]: '동물성 단백질 대비 적은 포화지방'과 같이 경쟁 카테고리를 깎아내리는 부당 비교는 면책 문구 유무와 상관없이 "수정 권고(부당 비교)" 조치하십시오.

    🔥 4. [영양강조표시 및 식약처 공식 기능성 명칭 검증]: 
       - 영양성분 '풍부'는 1일 기준치의 20% 이상, '함유/고단백'은 10% 이상인지 계산하여 검증하십시오.
       - 비타민, 칼슘 등 영양소의 기능성 설명 시 식약처 공전 워딩(예: 비타민E '항산화작용을 하여 유해산소로부터 세포를 보호')이 정확한지 대조하고 다르면 수정 권고하십시오.

    🔥 5. [출처 엄격 분리 및 식약처 DB 제한 적용]:
       - 제공된 [외부 DB 데이터]는 시안 내에 **'식약처' 또는 '식품의약품안전처'**가 기재된 구간에만 한정하여 적용하십시오. 타 기관 출처(보건복지부 등)는 억지 대조하지 마십시오.
       - '식약처' 출처가 확인된 구간에서 수치 비교가 있다면 환산 비례식을 세워 [비교 항목 | 시안 표기 수치 | 식약처 DB 실제 수치 | 일치 여부] 4칸짜리 표를 생성하십시오.

    🔥 6. [객관적 주장 실증 및 원물 은폐 기만]:
       - 메인 카피로 강조된 농산물이 팩시안에서 극소량(1% 미만 등)임에도 '풍미/고소함'을 과장했다면 기만으로 적발하십시오.
       - '2009년 출시', '12년 연속 1위' 등 객관적 주장에 대한 출처 표기가 시안 내에 없다면 근거 자료 구비를 권고하십시오.
    
    반드시 아래의 JSON 배열(Array) 형식으로만 응답하십시오. `raw_text_evidence` 칸에 당신이 이미지에서 읽은 원문을 그대로 작성한 뒤 분석을 시작하십시오.
    [
      {{
        "image_index": 구간 인덱스 번호 (0부터 시작),
        "risk_level": "치명적 위반" 또는 "수정 권고" 또는 "정상",
        "title": "검토 항목 요약",
        "raw_text_evidence": "시안 또는 팩시안에서 당신이 실제로 읽은 원문 그대로의 텍스트 (예: 콜레스테롤 0mg 33%, 검은콩농축액 0.21%)",
        "marketing_text": "상세페이지 추출 원문",
        "fact_or_legal_ground": "적용된 QC 룰 또는 식약처 규정",
...
if not uploaded_main_images:
    ...
    if not trigger_api:
        ...
        with st.spinner("⚙️ 2단계: 선임자급 범용 룰 및 오타 색출 정밀 심사 가동 중..."):
            try:
                ref_files = []
                    ...
                    with row_col2:
                        ...
                        if not issues:
                            ...
                            for issue in issues:
                                ...
                                # 원시 데이터 증거 출력 추가
                                if issue.get("raw_text_evidence"):
                                    st.markdown(f"**🔍 AI 스캔 원문:** `{issue.get('raw_text_evidence')}`")
