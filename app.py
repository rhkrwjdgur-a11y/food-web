...
# 4. 실시간 AI 비전 분석 로직 (강제 체크리스트 룰셋 탑재)
def analyze_design_with_ai(main_images, ref_files, master_fact_files, legal_text, db_context_text):
    ...
    # 팩트: AI가 칼로리 대조나 오탈자 스캔을 건너뛰지 못하도록 3가지 범주를 무조건 검토하게 JSON 배열을 강제함
    prompt = f"""
    ...
    [필수 체크리스트 대원칙 - 하나의 항목이라도 스킵 시 시스템 오류 발생]
    각 이미지 인덱스(조각)마다 아래의 3가지 카테고리를 독립적으로 무조건 스캔하고, 위반이 발견된 카테고리만 결과 배열에 포함하십시오.
    
    🔥 Category 1 (수치 및 팩트 무결성): 
       - 상세페이지 상/하단 간 칼로리, 내용량 등 수치가 1이라도 다르게 적힌 내부 모순이 없는가? (예: 상단 150kcal vs 하단 155kcal 등)
       - 마케팅 비교 수치가 [실시간 국가 공인 영양성분 DB] 수치와 소수점까지 일치하는가?
    
    🔥 Category 2 (텍스트 오탈자 및 누락): 
       - 하단 정보표시면(고시표)의 텍스트가 '확정 팩시안' 원문과 한 글자도 빠짐없이 100% 일치하는가? (예: '얌뿌리'를 '암뿌리'로 적는 등의 오타 색출)
       - 알레르기 성분 등 필수 기재사항이 누락되지 않았는가?
       
    🔥 Category 3 (법적 규제 및 연출 이미지): 
       - 조리된 형태의 연출된 사진 주변에 '연출된 이미지', '이미지 예' 등의 면책 문구가 기재되어 있는가?
       - '100%', '무첨가' 등 과장 광고 문구가 식약처 법령을 위반하는가?
    ...
    [
      {{
        ...
        "title": "검토 항목 요약 (예: 칼로리 수치 내부 모순, 원재료 오탈자 발견, 연출 이미지 문구 누락 등)",
        "marketing_text": "상세페이지에서 추출한 문제의 텍스트 또는 수치 원문",
        "fact_or_legal_ground": "팩시안 원문, 외부 DB, 또는 적용된 법적 가이드라인",
        "discrepancy_analysis": "위반 사항 분석 및 QC 실무 조치 사항"
      }}
    ]
    * 만약 해당 구간(image_index)에 3가지 카테고리 중 단 하나의 위반 사항도 없다면, 빈 배열 [] 을 반환하지 말고 risk_level을 "정상"으로 하여 1개의 객체를 반환하십시오.
    """
...
if not uploaded_main_images:
    ...
    if not trigger_api:
        ...
        with st.spinner("구글 Vision API 가동 중: 수치, 오탈자, 이미지 규정을 강제 체크리스트 기반으로 스캔하고 있습니다..."):
            try:
                db_context_text = ""
                if db_search_keyword:
                    keywords = [k.strip() for k in db_search_keyword.split(",")]
                    for kw in keywords:
                        if kw:
                            db_data = query_food_nutrient_db(kw)
                            if db_data:
                                db_context_text += f"\n[검색어 '{kw}'에 대한 식약처 공인 데이터]\n" + json.dumps(db_data, ensure_ascii=False) + "\n"
                                st.sidebar.success(f"✅ 식약처 DB '{kw}' 데이터 연동 완료")
                            else:
                                st.sidebar.error(f"❌ 식약처 DB에서 '{kw}' 데이터를 찾을 수 없습니다.")
