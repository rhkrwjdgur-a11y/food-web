...
def analyze_design_with_ai(main_images, ocr_extracted_text, db_context_text):
    ...
    for idx, img_obj in enumerate(main_images):
        ...
        # --- [1단계] 시안 텍스트 강제 원형 추출 ---
        extract_prompt = """
        당신은 텍스트 추출기입니다. 이 이미지에 적힌 모든 텍스트와 숫자를 '보이는 그대로' 한 글자의 수정 없이 필사하십시오.
        디자이너의 오타(무규 포장, 콜레스테롤 0mg 33% 등)를 절대 문맥에 맞게 고치지 말고 날것 그대로 추출하십시오.
        """
        try:
...
if not uploaded_main_images:
    ...
    if not trigger_api:
        ...
        with st.spinner("⚖️ [제2 에이전트] 불필요한 지적은 배제하고 핵심 오타 및 표기 위반을 색출합니다..."):
            try:
